...
\cite{belouadah2019il2m,wu2019bias_correction,zhao2020weightalignement}. The loss is the
binary-cross entropy. The independent classifiers paradigm coupled with the sigmoid activation and
binary cross-entropy loss exclude explicitly a late fusion \cite{ramachandram2017multimodalreview}
...
\begin{algorithm}[tb]
    ...
    \begin{algorithmic}[1]
        ...
        \Comment{\autoref{sec:vit}}

        \For{\texttt{$i \gets 1$; $i \leq t$; $i{+}{+}$}} \State $e_i \gets \operatorname{TAB}([\theta_i,
                x_L])$ \Comment{\autoref{sec:tab}} \State $\hat{y}_i \gets \operatorname{Clf}_i(e_i)$
        \Comment{\autoref{sec:ensembling_cab}} \EndFor
...
\paragraph{Benchmarks \& Metrics} We evaluate our model on CIFAR100 \cite{krizhevskycifar100},
ImageNet100 and ImageNet1000 \cite{deng2009imagenet} (descriptions in the supplementary materials)
...
defined by \cite{rebuffi2017icarl}. We also report the final accuracy after the last step
