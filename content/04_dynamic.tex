...
our framework on the recent Transformer architecture \autoref{sec:related_cv}.

\paragraph{Continual dynamic networks}  Notably, most dynamic strategies require the sample's task
...
%Despite being successful across various benchmarks, transformers have not
...
encoder made of Self-Attention Blocks, and the classifier. We described in the related work
(\autoref{sec:related_cv}) the general architecture. We denote in this chapter a Self-Attention
Block by its acronym: SAB, and we depict it in \autoref{fig:dytox_sab}.

\begin{figure}
    \centering
    \includegraphics[width=0\textwidth]{images/dytox/sab.pdf}
...
\begin{figure*}
    ...
    through the TAB is modified by a different task-specialized token $\theta_t\, \text{for}\, t \in
        \{1 \dots T\}$ (\autoref{sec:dytox_ensembling_cab}). The $T$ final embeddings are finally given
...
limited (+1.79 \textit{p.p.}). On the other hand, those two methods greatly improve the extreme
continual setting of 50 steps (+6.42 \textit{p.p.}). This shows that the gain is not due to absolute
