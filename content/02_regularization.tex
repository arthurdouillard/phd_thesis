\chapter{Visual Feature-based Regularizations for Incremental Classification}
...
We design a framework, nicknamed \textit{Ghost}, to exploit these class attributes estimate the
...
\paragraph{Complete classifier} Remind that the parameters  $\{\theta_c\,,\forall c \in
    \mcC^{1:t}\}$ on the representation-based classifier (\autoref{eq:podnet_lsc}) may be interpreted as
proxies for the classes $\mcC^{1:t}$. The base model for task $t$ will, thus, learn $\mcN^{1:t} =
    |\mcC^{1:t}|$ such proxies, one for each of the seen classes. To extend the model for the unseen
future classes, the complete classifier will learn $\mcN^{1:t} + \mcN^{t+1:T}$ proxies, which
changes \autoref{eq:podnet_nca} to:
%
\begin{equation}
...
markedly less challenging — setting appearing in \citet{pkankuekul2012onlineincrementalzeroshot} and
