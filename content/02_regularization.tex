\chapter{Visual Feature-based Regularizations}
...
\begin{figure}[tb]
    ...
    \caption{\textbf{Training protocol for incremental learning}. At each training task we learn a
        ...
        the first task contain more classes than the following tasks (50 \vs 10 on this figure).}
...
\end{equation}
%
\noindent or, contrarily, by pooling the space (equivalent, up to a factor, to a Global Average
Pooling), one preserves \textit{only} the channels:
%
\begin{equation}
...
\end{equation}
%
\noindent or, likewise, for the vertical dimension, resulting in POD-height. Each of those variants
measure the distribution of activation pixels across their respective axis. These two complementary
...
As we will see in our evaluations of pooling strategies (\autoref{tab:podnet_ablation_perceptual}),
...
\autoref{tab:podnet_ablation_memorysize}.
...
method introduced for UCIR (see \autoref{sec:podnet_related}). The training procedure is identical
for both inference scheme, their difference lies at test-time: ``CNN'' uses the
$\operatorname{argmax}$ of all classes probabilities. ``NME'' extracts features of all data (new
classes data and rehearsal data), builds class prototypes by averaging features per class, and
finally predictions using a K-NN.
%
...
only 20 images per class are kept in the memory. Remark that when few images per class can be
stored, a continual model must be rigid (to avoid forgetting old classes), while when numerous
images can be stored, a continual model must be plastic (to learn efficiently new classes and stored
examples). UCIR \citep{hou2019ucir} and BiC \citep{wu2019bias_correction} are respectively rigid-
and plastic-oriented models, which can be seen from their good performance with respectively small
and large rehearsal memory. On the other hand, PODNet excelled in all situations by efficiently
trading rigidity and plasticity with the POD loss. More experiments validating our model's robustness
...
We design a framework, nicknamed \textit{Ghost}, which estimates the representation of future, yet
...
\paragraph{Complete classifier} Remind that the parameters  $\{\theta_c\,,\forall c \in
    \mcC^{1:t}\}$ on the representation-based classifier (\autoref{eq:podnet_lsc}) may be
interpreted as proxies for the classes $\mcC^{1:t}$. The base model for task $t$ will, thus,
learn $\mcN^{1:t} = |\mcC^{1:t}|$ such proxies, one for each of the seen classes. To extend the
model for the unseen future classes, the complete classifier will learn $\mcN^{1:t} +
    \mcN^{t+1:T}$ proxies, which changes \autoref{eq:podnet_nca} to:
%
\begin{equation}
...
between sample feature $\vh_y$ and correct class proxy $\theta_y$ in the numerator. In the
denominator, the loss pushes away all wrong class proxies, from both seen and unseen (future)
classes, by minimizing the similarities with $\vy_c, \, \forall c \neq y$.
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
        ...
        \If{$t = 1$ or $t = T$} \State Train $f^t$ and $g^t$ with $\mcL^\text{\tiny{nca}}$ and
        ...
        \If{$t \ne 1$ and $t < T - 1$} \State Train $g^t$ with $\mcL^\text{\tiny{MMD}}$ using
...
\paragraph{Generator} Following the work of \citet{bucher2017zeroshot_gmmn} for zero-shot learning,
...
markedly less challenging — setting appearing in \citet{kankuekul2012onlineincrementalzeroshot} and
\citet{xue2017incrementalzeroshot}, where the set of unseen classes is fixed, and only more seen
