...
Neural Network are based on the statistical learning theory \citep{vapnik1999statstheory}. In the
...
$\mathcal{Y}$.  We consider a \textit{loss} function $\mathcal{L}: \mathcal{Y} \times \mathcal{Y} \rightarrow
...
with highest predicted probability/confidence is chosen as the prediction. Training a neural network
means that given a \textit{training dataset} $\{(\vx_1, y_1), \etc, (\vx_N, y_N)}$.

In practice, the function $f$ learned by a neural network is composed of a succession of linear
