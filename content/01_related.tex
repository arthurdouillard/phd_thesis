...
In this chapter, I detail the related work needed to read this thesis. I will first briefly
explain the learning procedure in \acf{DL} and how the data are structured. Then, I will describe how
\ac{DL} applications and architectures in \acf{CV}. Finally, I will introduce the main topic of this thesis
...
\citep{benyounes2017mutan}, \etc. I describe now in more details the structure of modern
\acp{CNN}, how to train them efficiently. Then, I highlight a contender to \acp{CNN}: the
...
structure and a \textit{self-attention} between the words embeddings of a sentence. Each word is
embedded into a high-dimensional vectors named a ``\textit{token}''. The self-attention of a
...
reducing greatly the number of tokens. More precisely, the tokens are computed using a
convolution whose stride is equal to the kernel size, producing patch embeddings without overlap.
This architecture is illustrated in \autoref{fig:related_vit}.
...
A special learned token, called ``\textit{class token}'', is added to the patch tokens. Then,
successive transformer blocks process all the tokens. Each block is made of LayerNorms
\citep{ba2016layernorm}, a self-attention, a \ac{MLP}, and residual connections. Thus, the self-attention
is:
%
\begin{equation}
      \begin{aligned}
...
among all classes it has learned. The \autoref{fig:related_forgetting} illustrate such continual
training. The \textcolor{orange}{orange} line displays the accuracy of a model which is re-trained
from scratch at each step on all previous classes data. This model, usually called Joint, is
...
classes. While I have mainly tackled \acf{CIL} benchmarks, I describe other Continual benchmarks in
...
Other metrics exist \citep{diaz2018continualmetrics}, including the \textbf{backwardø and
...
thesis, across three chapters and five published papers, I tackled multiple types of continual drift using
different approaches summarized below:

\paragraph{Feature-based Regularizations} First in \autoref{chapter:regularization}, I consider
...
probabilities (\autoref{sec:related_regul_output}). In this chapter, I show that this type
constraint struggle to balance efficiently the rigidity needed to not forget with the plasticity
required to still learn new classes. Therefore, I propose to regularize, not the final outputs, but
rather the intermediate feature space. In the first section (\autoref{sec:podnet}), I describe how
...
happen: Inspired by \ac{ZSL}, I estimate the feature representation of future classes using weak
...
\paragraph{Continual Semantic Segmentation} Second, in \autoref{chapter:segmentation}, I tackle
...
used for this type of data: first I used an uncertainty-based pseudo-labeling to handle what we
called a background shift. Then, I extend the feature regularization
(\autoref{sec:related_regul_output}) introduced in the previous chapter, POD, to constrain
differently local and global areas of the spatial feature maps. Finally, I consider rehearsal
...
\paragraph{Dynamic Transformers} In this third and last chapter (\autoref{chapter:dynamic}), I only
...
careful finetuning of hyperparameters. I propose in this chapter, a novel dynamic expansion with
