...
In this chapter, I detail the related work needed to read this thesis. I'll first briefly
...
\end{equation}
%
\noindent with $\vW_h \in \mathbb{R}^{H \times D}$, $\vb_h \in \mathbb{R}^{H}$,
...
Most \acs{CNN} follow a similar structure with blocks of convolutions and pooling. Usually, the
...
\paragraph{Architectures:}The 2010's decade saw major improvements to \acs{CNN}, both in their
...
enable the high performance of modern \acs{CNN}: optimizers with adaptive learning rate such as Adam
...
decade, in recent years, the transformer architecture gained interest: originally designed for
machine translation in \ac{NLP} \citep{vaswani2017transformer} with an encoder/decoder structure, it
has been applied to vision \citep{dosovitskiy2020vit} using a modified encoder-only structure as
BERT \citep{devlin2018bert}. A transformer's input is a sequence of ``\textit{tokens}'', which are
high-dimensional vectors. While \ac{NLP}, it used to be a learned embedding of sub-words, in
Vision it is a linear projection of patches of pixels.
...
\end{equation}
%
\noindent $\vx$ are the $N$ patch tokens and the class token, of shape $(N, D)$, $D$ being the embedding
...
images nor new classes will be learned. The knowledge acquired on one dataset A can be
\textit{transferred} to another dataset B with different classes using \textbf{transfer learning}
\citep{razavian2014transferlearning}. Then, the new model is efficient on
the dataset B but cannot classify anymore the classes of dataset A.

\textbf{Continual Learning} aims to learn a continually changing dataset without forgetting the
previous knowledge. The distribution of the dataset continually changes: at each time-step, \ac{NC},
...
learning an ever-growing dataset is doable: a naive but efficient approach consists in training from
...
the expanse of old data. Multiple distribution drifts exist in Continual Learning
\citep{morenotorresa2012datasetshift,lesort2021driftanalysis}, and they have been called under various names in the
literature. Given $x$ an input sample and $y$ its label, the major drifts are:
...
In the \autoref{chapter:regularization} and the \autoref{chapter:dynamic}, I tackled only the
prior drift, while in \autoref{chapter:segmentation} I solved all three described drifts.
...
classes data. This model, usually called Joint, is considered as a reasonable upper bound. On the
other hand, the \textcolor{blue}{blue} line is a model finetuned solely on new classes without
access to previous classes. The model's accuracy is considerably lower than its Joint
counterpart, because it completely forgets old classes by over-predicting new classes. While I have
mainly tackled \acf{CIL} benchmarks, other Continual benchmarks exist and are described in detail in
\autoref{sec:related_variation}.
...
this thesis, I focused on the Single-Head setting because it is more realistic as it is not always
possible to know from which step a sample come from in a real-life setting, and more challenging
...
Multiple other metrics exists \citep{diaz2018continualmetrics}, including the \textbf{backwardø and
...
Finally, metrics such as \textbf{speed} or used \textbf{capacity} are important:
...
review of old data (\autoref{sec:related_rehearsal}), regularizations constraining the model is
...
it would defeat the purpose of continual learning. \autoref{fig:related_protocol} illustrates how
...
Learning (\autoref{sec:related_online_learning}), that continual models still do not overfit. In the
...
applied depending on the number of samples per classes \citep{kim2020imbalancedcontinual}.
...
\cite{lopezpaz2017gem} proposed the GEM model that combines a constraint on the gradients and
...
proposed to constraint instead the statistics of all intermediate features of the \ac{ConvNet}.
...
classifier with a cosine similarity, resulting in unit norm classifier weights.
